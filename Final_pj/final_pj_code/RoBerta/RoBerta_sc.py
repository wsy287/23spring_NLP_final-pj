...
import os
import numpy as np
save_path='model/RoBerta_sc/'
if not os.path.exists(save_path):
    os.makedirs(save_path)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
version='roberta-large'
tokenizer=RobertaTokenizerFast.from_pretrained(version)
model=RobertaForSequenceClassification.from_pretrained(version).to(device)
...
for epoch in range(max_epoch):
    ...
    if highest_accuracy <= val_accuracy:
        ...
        torch.save(model.state_dict(), save_dir + 'roberta_sc.pth')
