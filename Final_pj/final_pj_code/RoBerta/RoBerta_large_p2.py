...
import os
import numpy as np
save_path='model/RoBerta_large_p2/'
...
def encode_data(df, tokenizer):
    ...
    tokenizer:object
    """
    sentences_old = sum([df.iloc[i, 1:3].tolist() for i in range(len(df))], start=[])
    sentences=[]
    # for sentence_old in sentences_old:
    #     sentence='If '+sentence_old+' is in common sense ?'
    #     sentences.append(sentence)
    for sentence_old in sentences_old:
        sentence = 'If ' + sentence_old + ' is against common sense ?'
        sentences.append(sentence)
...
model=RobertaForMultipleChoice.from_pretrained('roberta-large').to(device)
...
for epoch in range(max_epoch):
    ...
    if highest_accuracy<=val_accuracy:
        ...
        torch.save(model.state_dict(),save_dir + 'roberta_large_p2.pth')
...
test(save_dir +'roberta_large_p2.pth')