...
import os
import numpy as np
save_path='model/ALbertSC/'
if not os.path.exists(save_path):
    os.makedirs(save_path)
device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
version='albert-xxlarge-v2'
tokenizer=AutoTokenizer.from_pretrained(version)
model=AutoModelForSequenceClassification.from_pretrained(version).to(device)
...
for epoch in range(max_epoch):
    ...
    if highest_accuracy <= val_accuracy:
        ...
        torch.save(model.state_dict(), save_dir + 'albert_sc.pth')
